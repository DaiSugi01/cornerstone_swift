...
class HabitCollectionViewController: UICollectionViewController {
    ...
    func updateCollectionView() {
        var itemsBySection = model.habitsByName.values.reduce(
            ...
            if model.favoriteHabits.contains(habit) {
                section = .favorites
                item = ViewModel.Item.init(habit: habit, isFavorite: true)
            } else {
                section = .category(habit.category)
                item = ViewModel.Item.init(habit: habit, isFavorite: false)
            }
    ...
    override func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        let config = UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
            ...
            let favoriteToggle = UIAction(title: item.isFavorite ? "Unfavoite" : "Favoite") { action in
