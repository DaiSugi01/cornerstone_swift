...
print("\(destroyer.name)'s position is \(destroyer.position) now")
destroyer.moveRight()
print("After moveRight(), \(destroyer.name)'s position is \(destroyer.position) now")
...
"""
let answer = """
    The reason is that falcon is a Spaceship type variable which does not have weapon property.
    \"Weapon\" is decleared only in Fighter class which inherited Spaceship.
    """
...
for _ in 0...11 {
    destroyer.fire()
    print("RemainingFirePower is \(destroyer.remainingFirePower) now.")
}
